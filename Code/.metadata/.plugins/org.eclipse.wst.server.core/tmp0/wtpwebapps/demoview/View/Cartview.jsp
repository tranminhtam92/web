           ...
           function changequantity(masp,quantity) {
               //$("#result").html('<img src="${pageContext.request.contextPath}/hinhanh/not-available.png" />');
                $.post('${pageContext.request.contextPath}/Cart?command=sua', {'masp':masp,'quantity':quantity}, function (data) {
                    $("#total").html( data.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,")+"đ");
                 });
           }
</script>
...
<% if(gh.countitem()>0){ %>
	...
	<% for (Map.Entry<String, Monhang> list : gh.getListitem().entrySet()) {%>
		...
		<td><%if(list.getValue().getKhuyenmai()>0) {%>
							<span class="amountnew"><fmt:formatNumber type="number" pattern="###,###" value="<%=list.getValue().getGia()-((list.getValue().getGia()*list.getValue().getKhuyenmai()))/100%>"/>đ/kg</span><span class="amountold" ><fmt:formatNumber type="number" pattern="###,###" value="<%=list.getValue().getGia()%>"/>đ/kg</span>
							<%}

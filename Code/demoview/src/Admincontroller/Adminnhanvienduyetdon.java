...
public class Adminnhanvienduyetdon extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(key.equals("view")){
			...
			Nhanvienduyetdon nv=new Nhanvienduyetdon(manv, tennv, diachi, sodienthoai, gioitinh,"");
			if(nvDAO.addlogin(manv, matkhau)!=1){
				nvDAO.addnhanvien(manv, tennv, gioitinh, diachi, sodienthoai);
				listnv=nvDAO.getallnhanvien();
				request.setAttribute("list_employee", listnv);
				RequestDispatcher dispatcher = request.getServletContext()
		                .getRequestDispatcher("/admin/Employee.jsp");
				...
				RequestDispatcher dispatcher = request.getServletContext()
		                .getRequestDispatcher("/admin/AddEmployee.jsp");
		        dispatcher.forward(request, response);
			}
		}
	}
